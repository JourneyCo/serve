...
export class ProjectListComponent implements OnInit {
  displayedColumns: string[] = ['title', 'startTime', 'endTime', 'capacity', 'actions'];
    ...
    this.projectService.getProjects().subscribe(
      (projects) => {
        ...
        if (validProjects.length > 0) {
          // Get average lat/lng for centering the map
          const totalLat = validProjects.reduce((sum, p) => sum + (p.latitude || 0), 0);
          const totalLng = validProjects.reduce((sum, p) => sum + (p.longitude || 0), 0);
  ...
  getCapacityPercentage(project: Project): number {
    return (project.currentRegistrations / project.maxCapacity) * 100;
  }
